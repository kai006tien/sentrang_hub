  ...
  showModal('➕ Thêm Thành Viên & Cấp Tài Khoản', `
    <form onsubmit="handleCreateMemberAndAccountSubmit(event)">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.85rem;margin-bottom:1.25rem;">
        <div><label>Ban hoạt động</label><select id="mem-dept"><option value="Ban Phong trào">Ban Phong trào</option><option value="Ban Truyền thông">Ban Truyền thông</option><option value="Ban Chuyên môn">Ban Chuyên môn</option><option value="Ban Chủ nhiệm">Ban Chủ nhiệm</option></select></div>
        <div><label>Chức danh & Vai trò *</label><select id="mem-pos"><option value="Thành viên">Thành viên</option><option value="Trưởng ban">Trưởng ban</option><option value="Phó Chủ nhiệm">Phó Chủ nhiệm</option><option value="Chủ nhiệm">Chủ nhiệm</option><option value="Thủ quỹ">Thủ quỹ</option></select></div>
      </div>
...
async function handleCreateMemberAndAccountSubmit(e) {
  ...
  try {
    ...
    const roleMap = { 'Thành viên':'role_thanh_vien', 'Trưởng ban':'role_truong_ban', 'Phó Chủ nhiệm':'role_pho_chu_nhiem', 'Chủ nhiệm':'role_chu_nhiem', 'Thủ quỹ':'role_thu_quy' };
      ...
      <form onsubmit="handleEditMemberSubmit(event, '${m.id}')">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.85rem;margin-bottom:0.85rem;">
          ...
          <div><label>Thế hệ</label><select id="edit-mem-gen">
            ...
            <option value="Gen 10" ${m.generation==='Gen 10'?'selected':''}>Gen 10</option>
          </select></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.85rem;margin-bottom:0.85rem;">
          <div><label>Ban hoạt động</label><select id="edit-mem-dept">
            <option value="Ban Phong trào" ${m.department==='Ban Phong trào'?'selected':''}>Ban Phong trào</option>
            <option value="Ban Truyền thông" ${m.department==='Ban Truyền thông'?'selected':''}>Ban Truyền thông</option>
            <option value="Ban Chuyên môn" ${m.department==='Ban Chuyên môn'?'selected':''}>Ban Chuyên môn</option>
          ...
          <div><label>Chức danh & Vai trò</label><select id="edit-mem-pos">
            ...
            <option value="Trưởng ban" ${m.current_position==='Trưởng ban'?'selected':''}>Trưởng ban</option>
...
async function handleEditMemberSubmit(e, memberId) {
  ...
  try {
    ...
    const payload = {
      ...
      generation: document.getElementById('edit-mem-gen').value,
